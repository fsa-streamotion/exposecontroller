...
func New(exposer, domain, internalDomain, aliasDomain, urltemplate, nodeIP, routeHost, pathMode string, routeUsePath, http, tlsAcme bool, tlsSecretName string, tlsUseWildcard bool, ingressClass string, client *client.Client, restClientConfig *restclient.Config, encoder runtime.Encoder) (ExposeStrategy, error) {
	...
	case "ingress":
		...
		strategy, err := NewIngressStrategy(client, encoder, domain, aliasDomain, internalDomain, http, tlsAcme, tlsSecretName, tlsUseWildcard, urltemplate, pathMode, ingressClass)
		if err != nil {
	...
	case "route":
		...
		return strategy, nil
	case "":
		strategy, err := NewAutoStrategy(exposer, domain, aliasDomain, internalDomain, urltemplate, nodeIP, routeHost, pathMode, routeUsePath, http, tlsAcme, tlsSecretName, tlsUseWildcard, ingressClass, client, restClientConfig, encoder)
		if err != nil {
