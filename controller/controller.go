...
func NewController(
	...
	strategy, err := exposestrategy.New(config.Exposer, config.Domain, config.AliasDomain, config.InternalDomain, config.UrlTemplate, config.NodeIP, config.RouteHost, config.PathMode, config.RouteUsePath, config.HTTP, config.TLSAcme, config.TLSSecretName, config.TLSUseWildcard, config.IngressClass, kubeClient, restClientConfig, encoder)
	if err != nil {
